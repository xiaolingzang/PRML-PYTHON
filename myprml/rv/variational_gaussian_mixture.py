...
class VariationalGaussianMixture(RandomVariable):
    ...
    def _variational_expection(self, X):
        ...
        ln_Lambda = digamma(0.5 * (self.dof - np.arange(self.ndim)[:, None])).sum(axis=0) + self.ndim * np.log(2) + \
                    np.linalg.slogdet(self.W)[1]
        ln_r = ln_pi + 0.5 * ln_Lambda + maha_sq
        ln_r -= logsumexp(ln_r, axis=-1)[:, None]  #log(sum(exp(a)))
        r = np.exp(ln_r)
        return r
    ...
    def classify(self, X):
        ...
        :param X: (sample_size, ndim) ndarray
        :return: (sample_size, n_components) ndarray
                index of maximum posterior of the latent variable
        """
        return np.argmax(self._variational_expection(X), 1)
    ...
    def classify_proba(self, X):
        ...
        """
        return self._variational_expection(X)
