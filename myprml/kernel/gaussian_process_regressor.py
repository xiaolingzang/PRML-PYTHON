...
class GaussianProcessRegressor(object):
    def __init__(self, kernel, beta=1.):
        ...
        :param beta: precision parameter of observation noise
        """
        self.kernel = kernel
        self.beta = beta
    ...
    def fit(self, X, t, iter_max=0, learning_rate=0.1):
        ...
        :param X: ndarray (sample_size, n_features)
        :param t: ndarray (sample_size,)
        :param iter_max: maximum number of iterations updating hyperparameters
        :param learning_rate:  updation coefficient
        :return: list of log likelihood value at each iteration
        ...
        for i in range(iter_max):
            ...
            updates = np.array(
                [-np.trace(self.precision.dot(grad)) + t.dot(self.precision.dot(grad).dot(self.precision).dot(t)) for
                 grad in gradients]
            )
    ...
    def predict(self, X, with_error=False):
        ...
        :param X: ndarray (sample_size, n_features)
        :param with_error:
        :return: predictions of corresponding inputs
        """
        if X.ndim == 1:
            X = X[:, None]
