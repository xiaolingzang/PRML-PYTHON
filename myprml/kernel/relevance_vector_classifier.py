...
class RelevanceVectorClassifier(object):
    def __init__(self, kernel, alpha=1.):
        ...
        :param kernel:  kernel function to compute components of feature vectors
        :param alpha: initial precision of prior weight distribution
        """
        self.kernel = kernel
        self.alpha = alpha
    ...
    def fit(self, X, t, iter_max=100):
        ...
        :param iter_max: int     maximum number of iterations
        ...
        mean = mean[mask]
        self.mean, self.covriance = self._map_estimate(Phi, self.t, mean, 100)
    ...
    def predict_proba(self, X):
        ...
        phi = self.kernel(X, self.X)
        mu_a = phi @ self.mean
        var_a = np.sum(phi @ self.covriance * phi, axis=-1)
