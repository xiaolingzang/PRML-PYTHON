...
class PCA(object):
    def __init__(self, n_components):
        """
        construct principal component ananlysis
    ...
    def fit(self, X, method="eigen", iter_max=100):
        ...
        :param method: method to estimate the parameters  ["eigen", "em"]
        :param iter_max: maximum number of iterations for em algorithm
        ...
        if method not in method_list:
            print("avaliable methods are {}".format(method_list))
        self.mean = np.mean(X, axis=0)
        getattr(self, method)(X - self.mean, iter_max)

    def eigen(self, X, *args):
        sample_size, n_feature = X.shape
        if sample_size >= n_feature:
            cov = np.cov(X, rowvar=False)
            values, vectors = np.linalg.eigh(cov)
            index = n_feature - self.n_components
        else:
            cov = np.cov(X)
            values, vectors = np.linalg.eigh(cov)
        ...
        self.C = self.W @ self.W.T + self.var * np.eye(n_feature)
        if index == 0:
            self.Cinv = np.linalg.inv(self.C)
        else:
            self.Cinv = np.eye(n_feature) / np.sqrt(self.var) - self.W @ np.linalg.inv(self.__M) @ self.W.T / self.var
    ...
    def transform(self, X):
        ...
        :param X: (sample_size, n_features) ndarray
        :return: (sample_size, n_components) ndarray
                projcted input data
        """
    ...
    def proba(self, X):
        ...
        :param X: (sample_size, n_features) ndarray
                input data
        :return:  value of the marginal distribution
        """
        d = X - self.mean
        return (
