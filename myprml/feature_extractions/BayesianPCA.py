import numpy as np
from myprml.feature_extractions.pca import PCA
...
class BayesianPCA(PCA):
    def fit(self, X, iter_max=100, initial="random"):
        ...
        if initial not in initial_list:
            print("available initializatons are {}".format(initial_list))
        if initial == "random":
    ...
    def _maximization(self, X, Ez, Ezz):
        self.W = X.T @ Ez.dot(np.linalg.inv(np.sum(Ezz, axis=0) + self.var * np.diag(self.alpha)))
        self.var = np.mean(
            np.mean(X ** 2, axis=-1)
